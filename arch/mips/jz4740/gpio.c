...
void jz_gpio_write_bit(unsigned int gpio, unsigned int reg)
{
	writel(GPIO_TO_BIT(gpio), GPIO_TO_REG(gpio, reg));
}

EXPORT_SYMBOL_GPL(jz_gpio_write_bit);
...
void jz_gpio_set_irq_bit(unsigned int irq, unsigned int reg)
{
