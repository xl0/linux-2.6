...
static int jz_mmc_set_clock(u32 rate)
{
	...
	jz_mmc_stop_clock();
	__cpm_select_msc_clk(0,1);	/* select clock source from CPM */
	clkrt = jz_mmc_calc_clkrt(1, rate);
	REG_MSC_CLKRT(0) = clkrt;
	return MMC_NO_ERROR;
}
...
static void jz_mmc_receive_pio(struct jz_mmc_host *host)
{
	...
	for(count = 0; count < max; count++) {
		while (REG_MSC_STAT(0) & MSC_STAT_DATA_FIFO_EMPTY)
			;
		*buf++ = REG_MSC_RXFIFO(0);
	} 
	host->pio.len -= count;
...
#define RSP_TYPE(x)	((x) & ~(MMC_RSP_BUSY|MMC_RSP_OPCODE))
	...
	REG_MSC_IREG(0) = MSC_IREG_END_CMD_RES;	/* clear irq flag */
	if (cmd->opcode == 12) {
		while (timeout-- && !(REG_MSC_IREG(0) & MSC_IREG_PRG_DONE))
			;
		REG_MSC_IREG(0) = MSC_IREG_PRG_DONE;	/* clear status */
	}
	if (!mmc_slot_enable) {
...
static int jz_mmc_data_done(struct jz_mmc_host *host, unsigned int stat)
{
	...
	if (!data)
		return 0;
	REG_MSC_IREG(0) = MSC_IREG_DATA_TRAN_DONE;	/* clear status */
	jz_mmc_stop_clock();
...
static int jz_mmc_probe(struct platform_device *pdev)
{
	 ...
	 * with our defaults.
	 */
	retval = request_irq(IRQ_MSC0, jz_mmc_irq, 0, "MMC/SD", host);
	if (retval) {
	...
	rxdmachan =
		jz_request_dma(DMA_ID_MSC0_RX, "MMC Rx", jz_mmc_dma_rx_callback,
			       0, host);
	if (rxdmachan < 0) {
	...
	txdmachan =
		jz_request_dma(DMA_ID_MSC0_TX, "MMC Tx", jz_mmc_dma_tx_callback,
			       0, host);
	if (txdmachan < 0) {
