...
static struct jz4750_lcd_dma_desc *dma0_desc_palette, *dma0_desc_cmd, 
	*dma0_desc0, *dma0_desc1, *dma1_desc0, *dma1_desc1;
...
static int jz4750fb_ioctl(struct fb_info *info, unsigned int cmd, unsigned long arg)
{
	...
	case FBIODISPOFF:
		...
		if ( jz4750_lcd_info->panel.cfg & LCD_CFG_LCDPIN_SLCD )
			__lcd_clr_ena(); /* Smart lcd only support quick disable */
		else
...
static void jz4750fb_descriptor_init( struct jz4750lcd_info * lcd_info )
{
	...
	dma0_desc_cmd 		= dma_desc_base + 1;
	dma0_desc0 		= dma_desc_base + 2;
	dma0_desc1 		= dma_desc_base + 3;
	dma1_desc0 		= dma_desc_base + 4;
	dma1_desc1 		= dma_desc_base + 5;
	...
	if ( lcd_info->panel.cfg & LCD_CFG_TVEN ) /* TVE mode */
		...
		if (lcd_info->osd.fg0.bpp <= 8) /* load palette only once at setup?? */
			dma0_desc0->next_desc = (unsigned int)virt_to_phys(dma0_desc_palette);
...
{
#if defined(CONFIG_JZ4750_FUWA) /* FPGA test, pixdiv */
...
static int __init jz4750fb_init(void)
{
	...
	/* __gpio_as_lcd_18bit(); //gpio init in arch/mips/jz4750/board-fuwa.c */
